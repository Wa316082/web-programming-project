        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <h1 class="text-2xl font-semibold mb-6">{{ __('Create Product') }}</h1>
                    <form method="post" action="{{ route('product.store') }}" enctype="multipart/form-data" >
                        @csrf
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="">
                                <x-input-label for="category_id" :value="__('Sub Category')" />
                                <x-select-input id="sub_category_id" name="sub_category_id" :options="$subCategories->pluck('name', 'id')" :selected="old('Sub_category_id')" class="block mt-1 w-full" required />
                            ...
                            <div>
                                ...
                                <x-image-input id="image" class="block mt-1 w-full" name="image" :value="old('image')" required />
